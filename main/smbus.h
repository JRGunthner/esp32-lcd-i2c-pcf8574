...
typedef struct {
    bool init;              // true se a estrutura foi inicializada
...
/**
 * @brief Constroi uma nova instância de SMBus.
 *        A nova instância deve ser inicializada antes de chamar outras funções.
 * @return Ponteiro para a nova instância de SMBus, ou NULL se não puder ser criada.
 */
smbus_t* smbus_malloc(void);

/**
 * @brief Deleta uma instância SMBus existente.
 * @param[in,out] smbus Ponteiro para a instância SMBus que será deletada e definida como NULL.
 */
void smbus_free(smbus_t** smbus);

/**
 * @brief Inicializa uma instância SMBus com as informações I2C especificadas.
 *        O timeout I2C é definido como aproximadamente 1 segundo.
 * @param[in] smbus    Ponteiro para a instância SMBus inicializada.
 ...
 * @param[in]  cmd   Código de comando específico do dispositivo.
 * @param[out] data  Dados recebidos do dispositivo escravo.
 * @param[in]  len   Tamanho do array de dados. Se o escravo não fornecer bytes suficientes, ESP_ERR_TIMEOUT será
 * retornado.
 * @return ESP_OK se bem sucedido, ESP_FAIL ou ESP_ERR_* se ocorreu um erro.
 */
