/**
 * @brief
 * The LCD1602 controller is an HD44780-compatible controller that normally
 * operates via an 8-bit or 4-bit wide parallel bus.
 *
 * https://www.sparkfun.com/datasheets/LCD/HD44780.pdf
 *
 * The LCD1602 controller is connected to a PCF8574A I/O expander via the I2C
 * bus. Only the top four bits are connected to the controller's data lines. The
 * lower four bits are used as control lines:
 *
 *   - B7: data bit 3
 *   - B6: data bit 2
 *   - B5: data bit 1
 ...
 *   val |= 0x04                       // RS = 0 (command), RW = 0 (write), EN =
 * 1 i2c_write_byte(i2c_address, val) sleep(2ms) val &= 0xfb // EN = 0
 ...
 *   - no shift
 *
 * The controller must be set to 4-bit operation before proper communication can
 * commence. The initialisation sequence for 4-bit operation is:
 *
...
// not interrupt the 2 x nibble sequence.
...
esp_err_t i2c_lcd1602_reset(const i2c_lcd1602_info_t* i2c_lcd1602_info) {
    ...
    if ((last_err = _write_command(i2c_lcd1602_info,
                                    COMMAND_FUNCTION_SET | FLAG_FUNCTION_SET_MODE_4BIT |
                                    FLAG_FUNCTION_SET_LINES_2 | FLAG_FUNCTION_SET_DOTS_5X8)) !=
        ESP_OK) {
        if (first_err == ESP_OK)
            first_err = last_err;
    ...
    if ((last_err = _write_command(
            i2c_lcd1602_info, COMMAND_DISPLAY_CONTROL |
            i2c_lcd1602_info->display_control_flags)) !=
        ESP_OK) {
        if (first_err == ESP_OK)
            first_err = last_err;
...
esp_err_t i2c_lcd1602_set_display(i2c_lcd1602_info_t* i2c_lcd1602_info, bool enable) {
    ...
    if (_is_init(i2c_lcd1602_info)) {
        i2c_lcd1602_info->display_control_flags =
            _set_or_clear(i2c_lcd1602_info->display_control_flags, enable,
...
esp_err_t i2c_lcd1602_set_auto_scroll(i2c_lcd1602_info_t* i2c_lcd1602_info, bool enable) {
    ...
    if (_is_init(i2c_lcd1602_info)) {
        i2c_lcd1602_info->entry_mode_flags =
            _set_or_clear(i2c_lcd1602_info->entry_mode_flags, enable,
...
esp_err_t i2c_lcd1602_scroll_display_left(
    ...
    if (_is_init(i2c_lcd1602_info)) {
        // RAM is not changed
        err = _write_command(i2c_lcd1602_info,
                             COMMAND_SHIFT |
...
esp_err_t i2c_lcd1602_move_cursor_right(
    ...
    if (_is_init(i2c_lcd1602_info)) {
        // RAM is not changed. Shift direction is inverted.
        err = _write_command(i2c_lcd1602_info, COMMAND_SHIFT |
                                                   FLAG_SHIFT_MOVE_CURSOR |
                                                   FLAG_SHIFT_MOVE_LEFT);
    }
    return err;
...
esp_err_t i2c_lcd1602_define_char(const i2c_lcd1602_info_t* i2c_lcd1602_info,
    ...
    if (_is_init(i2c_lcd1602_info)) {
        index &=
            0x07;  // only the first 8 indexes can be used for custom characters
        err = _write_command(i2c_lcd1602_info,
                             COMMAND_SET_CGRAM_ADDR | (index << 3));
